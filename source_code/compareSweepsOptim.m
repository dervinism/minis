function [estimated, prctAmps, prctAmpsDev, prctAmpsNeg, prctAmpsNegDev, prctRTs, prctRTsDev, prctRTsNeg, prctRTsNegDev, prctTwoDs, prctTwoDsDev, prctTwoDsNeg,...
    ...
    lengthRatio] = compareSweepsOptim(sweepcount, filtfs, Amps, AmpsNeg, RTs, RTsNeg, TwoDs, TwoDsNeg, prct, prctNeg)
...
if (target + noise) > sweepcount || target == noise || target < 1 || noise < 1
    ...
    if (target + noise) > sweepcount
        uiwait(msgbox(...
            'Because the error bound cannot be estimated based on the data for the required file length, additional visualisation is not available.',...
            'modal'));
        return
    end
end
