...
if strcmpi(filename(end-2:end),'abf')
    if exist('pathname', 'var')
        ...
        hd = get_abf_header_info(filename);
    end
    hd.nADCchans = length(hd.fADCProgrammableGain);
    desired_chan = hd.nADCSamplingSeq(1);
    hd.sampled_chan_mask = 0*hd.sampled_chan_mask;
    hd.sampled_chan_mask(desired_chan+1) = 1;
...
%     end
    ...
    if exist('pathname', 'var')
        ...
        [abf_data,abf_hdr] = loadabfnew(filename,hd);
    end
    if nchans_to_read > 1 && length(hd.nADCSamplingSeq) > 1
        current = abf_data(:,2)'; % careful!
    else
        current = 0;
    end
else
...
noiseProperties.sweep = abf_data(:,1)';
...
noiseProperties.current = current;
end
